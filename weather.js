...
async function checkWeather(city) {
    ...
    if (response.status == 404) {
        ...
        document.querySelector(".cloud h1").innerHTML = Math.round(data.main.temp) + "°C";
        document.querySelector(".cloud h2").innerHTML = data.name;
        document.querySelector(".data .humidity p").innerHTML = data.main.humidity + "% <br />Humidity";
        document.querySelector(".data .wind p").innerHTML = data.wind.speed + " km/h <br />Wind Speed";
...
function getUserLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
            ...
            document.querySelector(".error").innerHTML = "Unable to access location. Please allow location access.";
            document.querySelector(".error").style.display = "block";
        });
    } else {
